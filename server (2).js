...
function memorySection(memory) {
  ...
  return [
    "",
    "HIS SAVED HISTORY WITH THIS SPECIFIC GIRL (past coaching sessions, oldest first — stored on his device):",
    memory,
    "Use this history actively: spot patterns (repeated flaking, fading or growing investment, who initiates, tests she keeps running), reference the trajectory in your read, and adjust the coaching accordingly. A third cancellation is not coached like a first one.",
  ].join("\n");
}

function buildGeneratePrompt({ her, stage, context, imageCount, memory }) {
  return [
    imageCount
    ...
    "",
    "THEN generate exactly 5 reply options, each a different flavor. Choose the 5 best fits for her message and the stage, drawn from this palette: playful tease · confident direct · scarcity/brevity (short, almost dismissive) · the date-setter (decisive concrete plan) · the curveball (unexpected, pattern-breaking, memorable) · push-pull (warmth and tease in the same breath, creates tension) · the callback (references something from earlier in their thread or his context — only if material exists) · the challenge (flips the frame so she's the one qualifying herself) · deadpan (bone-dry humor, zero effort shown). If her message is a test, at least one reply should pass it with playful agreement. If silence is recommended, the 5 replies become 'only if and when she reaches out again' options — never double texts. If her message signals genuine firm disinterest, the silence recommendation should be a permanent walk-away and you should say so.",
    "Tailor the coaching to the stage. Situationship: he keeps his value and options — never slides into unpaid-boyfriend duties (emotional support on demand, instant availability) while she keeps things undefined; if he wants more, the move is stating it once, calmly, in person — not via needy texts. Friend zone: less availability and less agreeableness, not more; he escalates playfully or exits, he does not orbit. She cancelled: one graceful, unbothered response, she proposes the new time — no interrogation, no visible disappointment. After an argument: no essays, no groveling; one calm line that de-escalates without surrendering his position, then move forward in person. Long-term relationship: frame stays but warmth leads — playful, appreciative, still a man with his own gravity; never score-keeping.",
    "",
...
function buildRatePrompt({ herLast, draft, memory }) {
  return [
    ...
    req.on("end", async () => {
      ...
      try {
        if (url === "/api/generate") {
          ...
          const memory = clip(body.memory, 4000);
          return send(res, 200, await callClaude(buildGeneratePrompt({ her, stage, context, imageCount: imageBlocks.length, memory }), imageBlocks));
        } else {
          const herLast = clip(body.herLast, 2000);
          const draft = clip(body.draft, 2000);
          if (!draft) return send(res, 400, { error: "Missing draft." });
          return send(res, 200, await callClaude(buildRatePrompt({ herLast, draft, memory: clip(body.memory, 4000) })));
        }
      } catch (e) {
